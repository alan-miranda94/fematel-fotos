...
const MyComponent = props => {
  ...
  const pressButton = async ()=>{
    ...
    const isInvetario = props.site.inventario.length > 0
  ...
  return (
    <TouchableOpacity style={styles.container} onPress={pressButton}>    
      ...
      <Text style={{fontWeight:'bold', fontSize:16, alignSelf:'center'}}>
        {props.site.name} | {props.site.list.length >0? props.site.type:'inventario'}
      </Text>
      </View>  
      
      <TouchableOpacity style={styles.texto} onPress={()=>props.remove(props.site['_id'])}>
            <Ionicons
